...
mode: autonomous
music: "Wait, SEA (symphonic) — user-supplied track, trimmed to the video's own 39.9s"
---
...
- **Watch scale**: every frame that shows the watch renders it hero scale, roughly 85-92% of the canvas height (up from a small demo inset in the previous cut), close enough in a couple of beats that the case or band may clip the frame edge — the way Apple's own device films hold a product close. Internal watch-face content (rings, rows, gauge) scales proportionally with the case; nothing about scene timing changes.
...
Scene 1 (0.0–1.2s): the drawn watch holds at center, ~88% of the frame height, its face showing the wordmark "two.ears" small in `primary` at the top like the real navigation title; as the VO says "Pick the room", a caption above the watch arrives: "Pick the room" in `text` white, display ramp `h3`.
