...
2. **NoiseFloorTracker** keeps a ring buffer of the last 100 window levels and reports the 10th percentile using the nearest-rank method: sort ascending and take the element at 1-based rank `ceil(0.10 * count)`, that is, zero-based index `ceil(0.10 * count) - 1`. No interpolation. Before the buffer has 20 windows it reports the minimum seen so far.
